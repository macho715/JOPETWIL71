...
import { getRuntimeCredentials, getUsageSnapshot, resolveGithubAuth, } from "./copilot-bridge.js";
            ...
            if (parsed.flags.has("json")) {
                ...
                if (snapshot.error) {
                    ...
                    for (const window of snapshot.windows) {
                        console.log(`- ${window.label}: ${window.remainingPercent.toFixed(2)}% left (${window.usedPercent.toFixed(2)}% used)`);
                    }
                }
            }
            return;
