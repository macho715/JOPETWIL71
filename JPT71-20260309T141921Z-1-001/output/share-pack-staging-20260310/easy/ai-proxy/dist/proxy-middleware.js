...
import { CopilotBridgeError, runPromptWithRetry, } from "./copilot-bridge.js";
import { scanMessagesForDlp } from "./dlp.js";
import { buildOperationalLog, toOperationalUsage, } from "./ops-log.js";
import { decideRoute, resolveDataSensitivity, } from "./routing.js";
const DEFAULT_MODEL = "github-copilot/gpt-5-mini";
const REQUEST_ID_HEADER = "x-request-id";
const CONTEXT_KEY = "__standaloneProxyContext";
function getProxyContext(res) {
    const record = res.locals;
    if (!record[CONTEXT_KEY] || typeof record[CONTEXT_KEY] !== "object") {
        record[CONTEXT_KEY] = {};
    }
    return record[CONTEXT_KEY];
}
function resolveRequestId(req) {
    const fromHeader = req.header(REQUEST_ID_HEADER);
    return fromHeader?.trim() ? fromHeader.trim() : randomUUID();
}
function ensureRequestMeta(req, ctx) {
    if (!ctx.requestId) {
...
function emitLog(params) {
    ...
    try {
        params.logger(buildOperationalLog({
...
function ensureChatRequest(req, res) {
...
function applySanitizedMessages(req, dlp) {
    const body = req.body;
...
function resolveSensitivity(req, routing) {
    if (routing) {
...
export function createChatProxyHandler(opts) {
    return async (req, res) => {
        ...
        try {
            ...
            const bridgeError = error instanceof CopilotBridgeError
                ? error
