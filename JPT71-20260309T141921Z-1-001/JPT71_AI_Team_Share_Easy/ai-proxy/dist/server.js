...
import { createChatProxyHandler, createPreSendDlpMiddleware, createRoutingGateMiddleware, } from "./proxy-middleware.js";
...
export function resolveServerOptionsFromEnv(env = process.env) {
    ...
    const allowSanitizedToCopilot = env.MYAGENT_PROXY_ALLOW_SANITIZED_TO_COPILOT?.trim() === "1";
    const authToken = env.MYAGENT_PROXY_AUTH_TOKEN?.trim() || "";
    const corsOrigins = Array.from(new Set([...DEFAULT_ALLOWED_ORIGINS, ...readCsvList(env.MYAGENT_PROXY_CORS_ORIGINS)]));
    return {
        host,
...
export function createStandaloneServer(opts) {
    const resolved = {
    ...
    const logger = resolved.enableOpsLogs
        ? (entry) => {
    ...
    app.use((req, res, next) => {
        ...
        res.setHeader("Access-Control-Allow-Headers", "Content-Type, x-request-id, x-ai-sensitivity, x-ai-proxy-token, Authorization");
        ...
        start() {
            return new Promise((resolve) => {
                const server = app.listen(resolved.port, resolved.host, () => {
                    ...
                    console.log(`allow sanitized to copilot: ${resolved.allowSanitizedToCopilot ? "on" : "off"}`);
