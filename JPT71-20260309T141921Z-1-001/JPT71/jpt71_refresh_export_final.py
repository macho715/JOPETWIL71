...
def create_final_sheets_in_workbook(refreshed_path: str, suffix: str = FINAL_SUFFIX) -> None:
    ...
    # Fallback for Excel constants not in pywin32 genpy (Excel 2010+)
    for name, val in (
        ("xlPasteColumnWidths", 8),
        ("xlPasteValuesAndNumberFormats", 12),
        ("xlPasteFormats", -4122),
    ):
        if not hasattr(xl, name):
            setattr(xl, name, val)
...
def main(in_path: str, out_refreshed: str):
    import os
    out_refreshed_abs = os.path.abspath(out_refreshed)
    ...
    wb.save(out_refreshed_abs)

    create_final_sheets_in_workbook(out_refreshed_abs)
